...
# Log out method to log out from the Moddle App
def log_out():
...
def create_new_user():
  ...
  # OR: Select(driver.find_element(By.ID, 'id_maildisplay')).select_by_value(0)
    ...
    'Canada')
    # OR Select(driver.find_element(By.ID, 'id_country')).select_by_value(
    #         'CA')
    sleep(0.25)
    Select(driver.find_element(By.ID, 'id_timezone')).select_by_visible_text('America/Vancouver')
    sleep(0.25)
    # Select(driver.find_element(By.ID, 'id_lange')).select_by_visible_text('EN')
    # sleep(0.25)
    ...
    sleep(0.25)
    # OR driver.find_element(By.XPATH, '//span[contains(., "Server files")]').click()
    #sleep(0.25)
    #driver.find_element(By.PARTIAL_LINK_TEXT, 'Server files').click()
    # can try link text too:
    ...
    # OR driver.find_element(By.ID, 'id_imagealt').send_keys(locators.pic_desc)
...
def delete_test_user():
    ...
    if driver.find_element(By.ID, 'fgroup_id_email_grp_label').is_displayed() and driver.find_element(By.ID, 'id_email').is_displayed():
        ...
        sleep(0.25)
        if driver.find_element(By.XPATH, f'//td[contains(.,"{locators.email}")]').is_displayed() and driver.find_element(By.XPATH, '//*[@title="Delete"]' ).is_displayed():
